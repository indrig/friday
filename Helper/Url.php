...
{
    /**
     * @var \Friday\Web\UrlManager URL manager to use for creating URLs
     */
    ...
    protected static function normalizeRoute($route)
    {
        ...
        if (strpos($route, '/') === false) {
            ...
            return $route === '' ? Friday::$app->controller->getRoute() : Friday::$app->controller->getUniqueId() . '/' . $route;
        } else {
            // relative to module
            return ltrim(Friday::$app->controller->module->getUniqueId() . '/' . $route, '/');
        }
    }
    ...
    public static function to($url = '', $scheme = false)
    {
        ...
        $url = AliasHelper::getAlias($url);
        if ($url === '') {
            $url = Friday::$app->getRequest()->getUrl();
        }
    ...
    public static function remember($url = '', $name = null)
    {
        ...
        if ($name === null) {
            Friday::$app->user->setReturnUrl($url);
        } else {
            Friday::$app->session->set($name, $url);
        }
    }
    ...
    {
        if ($name === null) {
            return Friday::$app->user->getReturnUrl();
        } else {
            return Friday::$app->session->get($name);
        }
    }
    ...
    {
        $params = Friday::$app->controller->actionParams;
        $params[0] = Friday::$app->controller->getRoute();
    ...
    {
        $currentParams = Friday::$app->request->get();
        $currentParams[0] = '/' . Friday::$app->controller->getRoute();
        $route = ArrayHelper::merge($currentParams, $params);
        return static::toRoute($route, $scheme);
    }

    /**
     * @return \Friday\Web\UrlManager URL manager used to create URLs
     * @since 2.0.8
     */
    protected static function getUrlManager()
    {
        return static::$urlManager ?: Friday::$app->urlManager;
    }
}
