...
use Friday\Db\ConnectionInterface;
use Yii;
...
class Transaction extends BaseObject
{
    ...
    /**
     * @var ConnectionInterface the database connection that this transaction is associated with.
     */
    public $connection;
    ...
    public function begin($isolationLevel = null)
    {
        ...
        if ($this->_level === 0) {
            if ($isolationLevel !== null) {
                $this->adapter->getSchema()->setTransactionIsolationLevel($isolationLevel);
            }
            \Friday::trace('Begin transaction' . ($isolationLevel ? ' with isolation level ' . $isolationLevel : ''), __METHOD__);

            $this->connection->trigger(Adapter::EVENT_BEGIN_TRANSACTION);
        ...
        if ($schema->supportsSavepoint()) {
            Friday::trace('Set savepoint ' . $this->_level, __METHOD__);
            $schema->createSavepoint('LEVEL' . $this->_level);
        } else {
            Friday::info('Transaction not started: nested transaction not supported', __METHOD__);
        }
        $this->_level++;
    }
    ...
    public function setIsolationLevel($level)
    {
        ...
        return;
        Friday::trace('Setting transaction isolation level to ' . $level, __METHOD__);
        $this->adapter->getSchema()->setTransactionIsolationLevel($level);
    }
}
