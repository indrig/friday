...
class Query extends Component implements QueryInterface
{
     ...
     * $query = (new  \Friday\Db\Query)->from(['u' => 'user', 'profile']);
     *
     * // SELECT * FROM (SELECT * FROM `user` WHERE `active` = 1) `activeusers`;
     * $subquery = (new  \Friday\Db\Query)->from('user')->where(['active' => true])
     * $query = (new  \Friday\Db\Query)->from(['activeusers' => $subquery]);
     *
