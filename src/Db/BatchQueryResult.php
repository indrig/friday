...
{
    /**
     * @var Adapter the DB connection to be used when performing batch query.
     ...
     * @return Awaitable array the data fetched
     */
    protected function fetchData() : Awaitable
    {
        if ($this->_dataReader === null) {
            $deferred = new Deferred();

            $this->query->createCommand($this->adapter)->query()->await(function ($dataReader) use ($deferred){
                if($dataReader instanceof Throwable){
                    $deferred->exception($dataReader);
                } else {
                    ...
                    $deferred->result($this->query->populate($rows));
                }
            });
            ...
            return AwaitableHelper::result($this->query->populate($rows));
        }
    }
