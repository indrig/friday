...
class ParameterContainer implements Iterator, ArrayAccess, Countable
{
     ...
     */
    public function offsetSet($name, $value, $type = null, $maxLength = null)
    {
        $position = false;
        ...
        if (is_int($name)) {
            ...
            throw new InvalidArgumentException('Keys must be string, integer or null');
        }
        if ($position === false) {
            $this->positions[] = $name;
        }
    ...
    public function offsetGetMaxLength($name)
    {
        ...
        return isset($this->maxLength[$name]) ? $this->maxLength[$name] : null;
    }
    /**
     ...
     * @param mixed $errata
     */
    public function offsetSetType($name, $errata)
    {
        if (is_int($name)) {
            $name = $this->positions[$name];
        }
        $this->types[$name] = $errata;
    }
    /**
     * Offset get errata
     *
     ...
     * @return mixed
     */
    public function offsetGetType($name)
    {
        if (is_int($name)) {
            $name = $this->positions[$name];
        }
     ...
     * @return bool
     */
    public function offsetHasType($name)
    {
        if (is_int($name)) {
            $name = $this->positions[$name];
        }
     ...
     * @throws InvalidArgumentException
     */
    public function offsetUnsetType($name)
    {
        if (is_int($name)) {
            $name = $this->positions[$name];
        }
    ...
    public function merge($parameters)
    {
        if (!is_array($parameters) && !$parameters instanceof ParameterContainer) {
            throw new InvalidArgumentException('$parameters must be an array or an instance of ParameterContainer');
        }
    ...
    public function bindParam($name, $value, $dataType = ParameterContainer::TYPE_AUTO, $length = null){
        ...
        $this->data[$name] = $value;
        if ($dataType) {
            $this->offsetSetType($name, $dataType);
        }
        if ($length) {
            $this->offsetSetMaxLength($name, $length);
        } else {
