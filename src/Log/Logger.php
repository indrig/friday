 ...
 *
 * A Logger instance can be accessed via `Friday::getLogger()`. You can call the method [[log()]] to record a single log message.
 * For convenience, a set of shortcut methods are provided for logging messages of various severity levels
 * via the [[Friday]] class:
 *
 * - [[Friday::trace()]]
 * - [[Friday::error()]]
 * - [[Friday::warning()]]
...
class Logger extends Component
{
    ...
    public function log($message, $level, $category = 'application')
    {
        ...
        if ($this->traceLevel > 0) {
            ...
            foreach ($ts as $trace) {
                if (isset($trace['file'], $trace['line']) && strpos($trace['file'], FRIDAY_PATH) !== 0) {
     ...
     * defined by constant `FRIDAY_BEGIN_TIME` which is evaluated at the beginning
     * of [[\Friday\AbstractFriday]] class file.
     * @return float the total elapsed time in seconds for current request.
     */
    public function getElapsedTime()
    {
        return microtime(true) - FRIDAY_BEGIN_TIME;
    }
     ...
     * For example, 'Friday\Db\*' will match categories starting with 'Friday\Db\',
     * such as 'Friday\Db\Connection'.
    ...
    {
        $timings = $this->getProfiling(['Friday\Db\Command::query', 'Friday\Db\Command::execute']);
        $count = count($timings);
        $time = 0;
