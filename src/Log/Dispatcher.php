 ...
 *
 * An instance of Dispatcher is registered as a core application component and can be accessed using `Friday::$app->log`.
 *
 ...
 *
 * ```php
 * Friday::$app->log->targets['file']->enabled = false;
 * ```
 *
 ...
 * @property Logger $logger The logger. If not set, [[\Friday::getLogger()]] will be used. Note that the type of
...
class Dispatcher extends Component
{
    ...
    public function init()
    {
        ...
        foreach ($this->targets as $name => $target) {
            if (!$target instanceof Target) {
                $this->targets[$name] = Friday::createObject($target);
            }
        }
    }
     ...
     * If not set, [[\Friday::getLogger()]] will be used.
     * @property Logger the logger. If not set, [[\Friday::getLogger()]] will be used.
     * @return Logger the logger.
     */
    public function getLogger()
    {
        if ($this->_logger === null) {
            $this->setLogger(Friday::getLogger());
        }
        return $this->_logger;
    }
     ...
     * or a configuration that will be used to create one using [[Friday::createObject()]].
     */
    public function setLogger($value)
    {
        if (is_string($value) || is_array($value)) {
            $value = Friday::createObject($value);
        }
        $this->_logger = $value;
        ...
        foreach ($this->targets as $target) {
            if ($target->enabled) {
                try {
                    ...
                    $targetErrors[] = [
                        'Unable to send log via ' . get_class($target) . ': ' . AbstractErrorHandler::convertExceptionToString($e),
