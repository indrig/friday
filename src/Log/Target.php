...
abstract class Target extends Component
{
    ...
    public function collect($messages, $final)
    {
        ...
        if ($count > 0 && ($final || $this->exportInterval > 0 && $count >= $this->exportInterval)) {
            if (($context = $this->getContextMessage()) !== '') {
                $this->messages[] = [$context, Logger::LEVEL_INFO, 'application', FRIDAY_BEGIN_TIME];
            }
    ...
    public function getMessagePrefix($message)
    {
        ...
        if(Friday::$app instanceof Friday\Web\Application){
            $request = Friday::$app->getRequest();
            $ip = $request instanceof Request ? $request->getUserIP() : '-';

            /*
            $user = Friday::$app->has('user', true) ? Friday::$app->get('user') : null;
            ...
            $session = Friday::$app->has('session', true) ? Friday::$app->get('session') : null;
