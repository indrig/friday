<?php
namespace Friday\Widget;

use Friday;
use Friday\Asset\ActiveFormAsset;
 ...
 */
class ActiveForm extends AbstractWidget
{
    /**
     * @var array|string $action the form action URL. This parameter will be processed by [[\Friday\Helper\Url::to()]].
     ...
     * @see fieldConfig
     */
    public $fieldClass = 'Friday\Widget\ActiveField';
    /**
    ...
    /**
     * @var boolean whether to hook up friday.activeForm JavaScript plugin.
     * This property must be set true if you want to support client validation and/or AJAX validation, or if you
     * want to take advantage of the friday.activeForm plugin. When this is false, the form will not generate
     * any JavaScript.
     */
     ...
     * @throws BadMethodCallException if `beginField()` and `endField()` calls are not matching
     */
    public function run()
    {
        if (!empty($this->_fields)) {
            throw new BadMethodCallException('Each beginField() should have a matching endField() call.');
        }
        ...
        if ($this->enableClientScript) {
            ...
            $view->registerJs("jQuery('#$id').fridayActiveForm($attributes, $options);");
        }
    ...
    protected function getClientOptions()
    {
        ...
        // only get the options that are different from the default ones (set in friday.activeForm.js)
     ...
     * be HTML-encoded using [[\Friday\Helper\Html::encode()]]. If a value is null, the corresponding attribute will not be rendered.
     ...
     * @return ActiveField|object the created ActiveField object
     * @see fieldConfig
     */
    public function field($model, $attribute, $options = [])
    {
        ...
        return Friday::createObject(ArrayHelper::merge($config, $options, [
            'model' => $model,
            'attribute' => $attribute,
            'form' => $this,
        ]));
     ...
     * @throws BadMethodCallException if this method is called without a prior [[beginField()]] call.
     */
    public function endField()
    {
        ...
        if ($field instanceof ActiveField) {
            ...
            throw new BadMethodCallException('Mismatching endField() call.');
        }
    }
     ...
     * $model = new Post;
     * $model->load($_POST);
     * if (Friday::$app->request->isAjax) {
     *     Friday::$app->response->format = Response::FORMAT_JSON;
