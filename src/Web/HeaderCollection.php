...
class HeaderCollection extends BaseObject implements \IteratorAggregate, \ArrayAccess, \Countable
{
     ...
     * @param boolean $last whether to only return the first header of the specified name.
     ...
     */
    public function get($name, $default = null, $last = true)
    {
        $name = strtolower($name);
        if (isset($this->_headers[$name])) {
            return $last ? end ($this->_headers[$name]) : $this->_headers[$name];
        } else {
            return $default;
        }
    }
