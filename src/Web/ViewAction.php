...
class ViewAction extends Action
{
    ...
    public function run()
    {
        ...
        $this->controller->actionParams[$this->viewParam] = Friday::$app->request->get($this->viewParam);
        ...
        try {
            ...
            if (FRIDAY_DEBUG) {
                ...
                throw new NotFoundHttpException(
                    Friday::t('The requested view "{name}" was not found.', ['name' => $viewName])
                );
            }
        }
    ...
    {
        $viewName = Friday::$app->request->get($this->viewParam, $this->defaultView);
        ...
        if (!is_string($viewName) || !preg_match('~^\w(?:(?!\/\.{0,2}\/)[\w\/\-\.])*$~', $viewName)) {
            if (FRIDAY_DEBUG) {
                ...
                throw new NotFoundHttpException(Friday::t('The requested view "{name}" was not found.', ['name' => $viewName]));
            }
        }
