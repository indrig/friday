...
class Session extends Component implements \IteratorAggregate, \ArrayAccess, \Countable
{
    ...
    public function init()
    {
        ...
        if ($this->getIsActive()) {
            Friday::warning("Session is already started", __METHOD__);
    ...
    public function open()
    {
        ...
        if ($this->getIsActive()) {
            Friday::info('Session started', __METHOD__);
            $this->updateFlashCounters();
        } else {
            $error = error_get_last();
            $message = isset($error['message']) ? $error['message'] : 'Failed to start session.';
            Friday::error($message, __METHOD__);
        }
    }

    /**
     * Registers session handler.
     * @throws \Friday\Base\Exception\InvalidConfigException
     */
    protected function registerSessionHandler()
    {
        if ($this->handler !== null) {
            if (!is_object($this->handler)) {
                $this->handler = Friday::createObject($this->handler);
            }
    ...
    {
        $path = AliasHelper::getAlias($value);
        if (is_dir($path)) {
