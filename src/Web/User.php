...
use Friday\Base\Exception\InvalidValueException;
use Friday\Rbac\CheckAccessInterface;
...
class User extends Component
{
    ...
    protected function loginByCookie()
    {
        ...
        if (isset($data['identity'], $data['duration'])) {
            ...
            if ($this->beforeLogin($identity, true, $duration)) {
                ...
                $id = $identity->getId();
                $ip = Friday::$app->getRequest()->getUserIP();
                Friday::info("User '$id' logged in from $ip via cookie.", __METHOD__);
    ...
    public function logout($destroySession = true)
    {
        ...
        if ($identity !== null && $this->beforeLogout($identity)) {
            ...
            $id = $identity->getId();
            $ip = Friday::$app->getRequest()->getUserIP();
            Friday::info("User '$id' logged out from $ip.", __METHOD__);
            if ($destroySession && $this->enableSession) {
                Friday::$app->getSession()->destroy();
            }
    ...
    {
        $url = Friday::$app->getSession()->get($this->returnUrlParam, $defaultUrl);
        if (is_array($url)) {
            if (isset($url[0])) {
                return Friday::$app->urlManager->createUrl($url);
            } else {
                $url = null;
            }
        }

        return $url === null ? Friday::$app->getHomeUrl() : $url;
    }
    ...
    {
        Friday::$app->getSession()->set($this->returnUrlParam, $url);
    }
    ...
    public function loginRequired($checkAjax = true, $checkAcceptHeader = true)
    {
        ...
        if ($this->loginUrl !== null && $canRedirect) {
            ...
            if ($loginUrl[0] !== Friday::$app->requestedRoute) {
                return Friday::$app->getResponse()->redirect($this->loginUrl);
            }
        }
        throw new ForbiddenHttpException(Friday::t('Login Required'));
    }
    ...
    protected function renewIdentityCookie()
    {
        ...
        $value = Friday::$app->getRequest()->getCookies()->getValue($name);
        if ($value !== null) {
            ...
            if (is_array($data) && isset($data[2])) {
                ...
                Friday::$app->response->getCookies()->add($cookie);
            }
        }
    }
    ...
    {
        $value = Friday::$app->response->getCookies()->getValue($this->identityCookie['name']);
        if ($value === null) {
            return null;
        }
        $data = json_decode($value, true);
        if (count($data) == 3) {
            ...
            if ($identity !== null) {
                if (!$identity instanceof IdentityInterface) {
                    throw new BadMethodCallException("$class::findIdentity() must return an object implementing IdentityInterface.");
                } elseif (!$identity->validateAuthKey($authKey)) {
                    Friday::warning("Invalid auth key attempted for user '$id': $authKey", __METHOD__);
                } else {
    ...
    {
        Friday::$app->getResponse()->getCookies()->remove(new Cookie($this->identityCookie));
    }
    ...
    {
        $acceptableTypes = Friday::$app->getRequest()->getAcceptableContentTypes();
    ...
    {
        return Friday::$app->getAuthManager();
    }
