...
class Module extends ServiceLocator
{
     ...
     * ```php
     * [
     *   'account' => 'Application\Controller\UserController',
     *   'article' => [
     *      'class' => 'Application\Controller\PostController',
    ...
    public static function getInstance()
    {
        ...
        return isset(Friday::$app->loadedModules[$class]) ? Friday::$app->loadedModules[$class] : null;
    }
    ...
    {
        if ($instance === null) {
            unset(Friday::$app->loadedModules[get_called_class()]);
        } else {
            Friday::$app->loadedModules[get_class($instance)] = $instance;
        }
    }
    ...
    {
        return AliasHelper::getAlias('@' . str_replace('\\', '/', $this->controllerNamespace));
    }
    ...
    {
        $this->_viewPath = AliasHelper::getAlias($path);
    }
    ...
    public function getLayoutPath() : string
    {
        if ($this->_layoutPath === null) {
            $this->_layoutPath = $this->getViewPath() . DIRECTORY_SEPARATOR . 'layout';
        }
    ...
    {
        $this->_layoutPath = AliasHelper::getAlias($path);
    }
    ...
    public function setAliases(array $aliases)
    {
        foreach ($aliases as $name => $alias) {
            AliasHelper::setAlias($name, $alias);
        }
    }
     ...
     */
    public function getModules(bool $loadedOnly = false) : array
    {
        if ($loadedOnly) {
            $modules = [];
    ...
    public function runAction($route, $params = [])
    {
        ...
        if (is_array($parts)) {
           ...
           $oldController = Friday::$app->controller;
            Friday::$app->controller = $controller;
            $result = $controller->runAction($actionID, $params);
            Friday::$app->controller = $oldController;
    ...
    public function createController($route)
    {
        ...
        if (isset($this->controllerMap[$id])) {
            $controller = Friday::createObject($this->controllerMap[$id], [$id, $this]);
    ...
    public function createControllerByID($id)
    {
        ...
        $prefix = str_replace(' ', '', ucwords(str_replace('-', ' ', $prefix), ' /'));
        ...
        if (is_subclass_of($className, 'Friday\Base\Controller')) {
            $controller = Friday::createObject($className, [$id, $this]);
            return get_class($controller) === $className ? $controller : null;
        } elseif (FRIDAY_DEBUG) {
            throw new InvalidConfigException("Controller class must extend from Friday\\Base\\Controller.");
        } else {
            return null;
        }
    }
