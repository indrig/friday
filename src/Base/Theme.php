...
class Theme extends Component
{
    ...
    {
        $this->_baseUrl = rtrim(AliasHelper::getAlias($url), '/');
    }
    ...
    {
        $this->_basePath = AliasHelper::getAlias($path);
    }
    ...
    public function applyTo($path)
    {
        ...
        if (empty($pathMap)) {
            ...
            $pathMap = [Friday::$app->getBasePath() => [$basePath]];
        }

        $path = FileHelper::normalizePath($path);

        foreach ($pathMap as $from => $tos) {
            $from = FileHelper::normalizePath(AliasHelper::getAlias($from)) . DIRECTORY_SEPARATOR;
            if (strpos($path, $from) === 0) {
                $n = strlen($from);
                foreach ((array) $tos as $to) {
                    $to = FileHelper::normalizePath(AliasHelper::getAlias($to)) . DIRECTORY_SEPARATOR;
