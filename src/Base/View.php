...
use Friday\Widget\Block;
use Friday\Widget\ContentDecorator;
use Friday\Widget\FragmentCache;
...
class View extends Component
{
    ...
    public function init()
    {
        ...
        if (is_array($this->theme)) {
            if (!isset($this->theme['class'])) {
                $this->theme['class'] = 'Friday\Base\Theme';
            }
            $this->theme = Friday::createObject($this->theme);
     ...
     * @throws InvalidArgumentException if the view cannot be resolved or the view file does not exist.
     * @see renderFile()
     */
     ...
     * @throws InvalidArgumentException if a relative view name is given while there is no active context to
    ...
    protected function findViewFile($view, $context = null)
    {
        if (strncmp($view, '@', 1) === 0) {
            // e.g. "@app/views/main"
            $file = AliasHelper::getAlias($view);
        } elseif (strncmp($view, '//', 2) === 0) {
            // e.g. "//layouts/main"
            $file = Friday::$app->getViewPath() . DIRECTORY_SEPARATOR . ltrim($view, '/');
        } elseif (strncmp($view, '/', 1) === 0) {
            // e.g. "/site/index"
            if (Friday::$app->controller !== null) {
                $file = Friday::$app->controller->module->getViewPath() . DIRECTORY_SEPARATOR . ltrim($view, '/');
            } else {
                throw new BadMethodCallException("Unable to locate view file for view '$view': no active controller.");
            }
        ...
        if ($this->defaultExtension !== 'phtml' && !is_file($path)) {
            $path = $file . '.phtml';
        }
    ...
    {
        $viewFile = AliasHelper::getAlias($viewFile);
        ...
        if ($this->beforeRender($viewFile, $params)) {
            Friday::trace("Rendering view file: $viewFile", __METHOD__);
            $ext = pathinfo($viewFile, PATHINFO_EXTENSION);
            if (isset($this->renderers[$ext])) {
                if (is_array($this->renderers[$ext]) || is_string($this->renderers[$ext])) {
                    $this->renderers[$ext] = Friday::createObject($this->renderers[$ext]);
                }
